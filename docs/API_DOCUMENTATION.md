...
- **Note**: there is currently no endpoint for staff to finalize the closure (set `Closed`) — this flow is partially implemented. See [TECHNICAL_DEBT.md](TECHNICAL_DEBT.md).
...
- Query: `accountId?` (must be owned by caller), `type?` (`transfer-debit` | `transfer-credit` | `deposit`), `status?`, `search?` (regex on `reference`), `from?`/`to?` (ISO dates, filters `createdAt`), `page?` (default 1), `limit?` (default 20, max 100)
